...
export function TrackModal({ track, isOpen, onClose, showBackButton = false, onBack }: TrackModalProps) {
  ...
  const availablePlatforms = PLATFORM_CONFIG.filter(platform => {
    const url = track[platform.key] as string;
    return url && url.trim() !== '';
  });
          ...
          <div className={`space-y-6 ${isMobile ? 'flex-1 overflow-y-auto' : ''}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="relative w-full aspect-square max-w-xs mx-auto">
                ...
                <div className={`w-full h-full bg-gray-800 rounded-xl flex items-center justify-center ${track.artworkUrl ? 'hidden' : ''}`}>
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-gray-400 text-sm">{track.credits || 'Credits and more'}</p>
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              {availablePlatforms.length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-400">
                  <p>No platform links configured for this track.</p>
                </div>
              )}
            </div>
          </div>
